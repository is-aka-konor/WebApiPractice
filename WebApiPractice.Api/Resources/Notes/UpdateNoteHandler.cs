...
namespace WebApiPractice.Api.Resources.Notes
{
    ...
    public class UpdateNoteHandler : IRequestHandler<UpdateNoteRequest, UpdateNoteResponse>
    {
        ...
        public UpdateNoteHandler(ICustomerRepository customerRepository
        ...
        {
            var cusomerExternalId = Guid.TryParse(request.CustomerExternalId, out var customerGuid) ? customerGuid : Guid.Empty;
            if (cusomerExternalId == Guid.Empty)
            {
                // If validation contracts were applied correctly then we should not be here
                this._logger.LogWarning($"An update note request with Guid {request.CustomerExternalId} as customer external id by pass validation. Please investigate.");
                throw new ResourceNotFoundException($"{ErrorCode.ResourceNotFound.Message} Resource Id: {request.CustomerExternalId}");
            }
            var customer = await this._customerRepository.GetCustomerByExternalId(cusomerExternalId).ConfigureAwait(false);
            if (customer is null)
            {
                // If validation contracts were applied correctly then we should not be here
                this._logger.LogWarning($"An update note request with customer external id: {request.CustomerExternalId} by pass validation. Please investigate.");
                throw new ResourceNotFoundException($"{ErrorCode.ResourceNotFound.Message} Resource Id: {request.CustomerExternalId}");
            }
            ...
            var note = await this._noteRepository.GetNoteByExternalId(noteExternalId).ConfigureAwait(false);
            if (note is null)
            {
                // If validation contracts were applied correctly then we should not be here
            ...
            note = await this._noteRepository.UpdateNote(note).ConfigureAwait(false);
